...
UI is built on `lockwright-lib-ui-react-native-components`. All new UI — v2 redesigns of existing screens and net-new features — **must** use components from this kit. Do not roll custom buttons, inputs, modals, typography, or icons.
...
- Import icons from `lockwright-lib-ui-react-native-components/icons` (133 available). Do not add new SVGs under `src/`.

**Migration state:** the extension is fully on `lockwright-lib-ui-react-native-components`. All screens and components use kit primitives. There is no `EXTENSION_DESIGN_VERSION` / `isV2()` runtime flag. A small number of legacy components remain in [src/shared/components/](src/shared/components/) (e.g. `Switch`, `SwitchWithLabel`, `TimerBar`) — do not grow that tree.
